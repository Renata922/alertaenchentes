...
const smtpTransporter = nodemailer.createTransport({
  ...
  requireTLS: String(process.env.EMAIL_REQUIRE_TLS ?? 'false').toLowerCase() === 'true',
  pool: true,
  maxConnections: 2,
  maxMessages: 20,
...
// --- GAS ---
async function sendViaGAS({ to, subject, html, text }) {
  const url = process.env.GAS_WEBHOOK_URL;
  const token = process.env.GAS_TOKEN;
  if (!url || !token) throw new Error('GAS_WEBHOOK_URL/GAS_TOKEN ausentes');
...
async function enviarEmail({ to, subject, html, text }) {
  const mode = String(process.env.EMAIL_MODE || '').toLowerCase(); // 'gas' | 'gmailapi' | 'smtp'(padrão)
  const asText = text || (html ? html.replace(/<[^>]+>/g, ' ') : '');

  // 1
  if (mode === 'gas')      return sendViaGAS({ to, subject, html, text: asText });
  if (mode === 'gmailapi') return sendViaGmailAPI({ to, subject, html, text: asText });

  // 2
  if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
    try {
      const from = process.env.EMAIL_FROM || process.env.EMAIL_USER;
      return await smtpTransporter.sendMail({ from, to, subject, text: asText, html });
    } catch (e) {
      console.warn('[EMAIL] SMTP falhou, tentando Gmail API...', e.message);
    }
  }

  if (process.env.GMAIL_CLIENT_ID && process.env.GMAIL_CLIENT_SECRET &&
      process.env.GMAIL_REFRESH_TOKEN && process.env.GMAIL_SENDER) {
    try {
      return await sendViaGmailAPI({ to, subject, html, text: asText });
    } catch (e) {
      console.warn('[EMAIL] Gmail API falhou, tentando GAS...', e.message);
    }
  }

  if (process.env.GAS_WEBHOOK_URL && process.env.GAS_TOKEN) {
    return sendViaGAS({ to, subject, html, text: asText });
  }

  throw new Error('Nenhuma configuração de e-mail válida encontrada (SMTP, Gmail API ou GAS).');
}
...
Para cancelar SMS, responda STOP.`;
      ...
      // E-mail (fire-and-forget por alerta)
      if (u.email){
        for (const a of alertas){
          ...
          `;
          setImmediate(() => {
            enviarEmail({ to: u.email, subject, html })
              .catch(e => console.error('❌ Falha e-mail p/ ', u.email, e.message));
          });
        }
      }
    }
  ...
  try {
    ...
    setImmediate(() => {
      enviarEmail({ to: user.email, subject, html })
        .catch(e => console.warn('Aviso: falha ao enviar e-mail /auth/forgot:', e.message));
    });

    return res.json({ sucesso:true, mensagem:'Se o e-mail existir em nossa base, você receberá o código em até 15 minutos.' });
  } catch (err) {
  ...
  try {
    ...
    setImmediate(() => {
      enviarEmail({
        ...
        `
      }).catch(e => console.warn('Aviso: falha ao enviar e-mail de confirmação de reset:', e.message));
    });
  ...
  try {
    ...
    setImmediate(() => {
      enviarEmail({
        ...
        `
      }).catch(e => console.warn('Aviso: falha ao enviar e-mail de confirmação de cadastro:', e.message));
    });
